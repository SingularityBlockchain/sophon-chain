...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/sophon-install-init sophon-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml sophon-install-init-"$TARGET"* $MAYBE_TARBALLS; do
